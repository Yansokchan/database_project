...
export const createCustomer = async (
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("customers")
      .insert([{ ...customer, id: newId, createdAt: new Date() }])
      .select()
      .single();
...
export const createEmployee = async (
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("employees")
      .insert([{ ...employee, id: newId, hireDate: new Date() }])
      .select()
      .single();
...
export const fetchOrderById = async (id: string): Promise<Order | null> => {
  ...
  try {
    ...
    if (!data) {
      console.error("No order data found with ID:", id);
      return null;
    }
...
export const createOrder = async (
  ...
  try {
    ...
    console.log("Fetched complete order:", completeOrder);
...
export const updateOrder = async (
  ...
  try {
    ...
    // Convert camelCase to snake_case for Supabase and ensure proper data types
    const updateData = {
    ...
    // Then update the order items in a separate table if they exist
    let items = [];
